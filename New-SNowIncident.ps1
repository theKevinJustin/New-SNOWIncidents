...
$CallerID = "13ad1d814fb68c25038e92468c676063c"
...
if ( $AlertResolutionState -ne 255 )
	{
	write-host "ServiceNow (SNow) Incident created with ResolutionState $($AlertResolutionState)"
	Add-Event "ServiceNow (SNow) Incident created with ResolutionState $($AlertResolutionState)"
	}
if ( $AlertResolutionState -eq 255 )
	{
	$TicketID = "Closed_NO_SNOW_Incident"
	write-host "ServiceNow (SNow) Incident NOT created as SCOM alert closed"
	Add-Event "ServiceNow (SNow) Incident NOT created as SCOM alert closed"
	exit $0
	}
...
if ( $TicketID -eq $Null )
	{
	$Alerts | Set-SCOMAlert -Owner $AssignmentGroup -ResolutionState $AlertResolutionState `
	-Comment "ServiceNow SCOM event automation - Set Owner, Resolution state in current alert $date"
	write-host "Updated SCOM alert for group $($AssignmentGroup)"
	Add-Event "Updated SCOM alert for group $($AssignmentGroup)"
	}
...
if ( $TicketID -ne $Null )
	{
	$Alerts | Set-SCOMAlert -ticketID $TicketID `
	-Owner "$AssignmentGroup"  -ResolutionState $AlertResolutionState `
	-Comment "ServiceNow SCOM event automation - Set Ticket, Owner, Resolution state in current alert $date"
	write-host "Updated SCOM alert for Ticket $($TicketID) group $($AssignmentGroup)"
	Add-Event "Updated SCOM alert for Ticket $($TicketID) group $($AssignmentGroup)"
	}
...
# Set ResolutionState
$AlertResolutionState = $Alerts[0].ResolutionState
...
$incidentData = @{
	...
	short_description = "##Company## ##Team## SCOM" + $AlertName
...
#write-host "ServiceNow Incident payload `n `n $($IncidentData)"
Add-Event "ServiceNow Incident payload `n `n $($IncidentData)"
...
try
	{
        ...
        #Write-Host "Attempting to create incident for $AlertName on $HostName..."
        Add-Event "Attempting to create incident for $AlertName on $HostName..."
